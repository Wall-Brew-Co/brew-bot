...
(deftest generate-beer-recipe-test
  (testing "Ensure the application can generate recipes that are sane"
    (let [gallons        (* (inc (rand-int 100)) 1.0)
      ...
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:grains random)))))
      (is (every? #(csa/valid? ::s/gravity %) (map :gravity (vals (:extracts random)))))
      (is (every? #(csa/valid? ::s/weight %) (map :weight (vals (:extracts random)))))
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:extracts random)))))
      (is (every? #(csa/valid? ::s/alpha %) (map :alpha (vals (:hops random)))))
      (is (every? #(csa/valid? ::s/beta %) (map :beta (vals (:hops random)))))
      (is (every? #(csa/valid? ::s/weight %) (map :weight (vals (:hops random)))))
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:hops random)))))
      (is (csa/valid? ::s/gravity (:gravity random)))
      ...
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:grains limited-random)))))
      ...
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:extracts limited-random)))))
      (is (every? #(csa/valid? ::s/alpha %) (map :alpha (vals (:hops limited-random)))))
      (is (every? #(csa/valid? ::s/beta %) (map :beta (vals (:hops limited-random)))))
      (is (every? #(csa/valid? ::s/weight %) (map :weight (vals (:hops limited-random)))))
      (is (every? #(csa/valid? ::s/name %) (map :name (vals (:hops limited-random)))))
      (is (csa/valid? ::s/gravity (:gravity limited-random))))))
