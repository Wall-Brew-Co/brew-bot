...
(deftest grain-validity-test
  (testing "Ensure all grain definitions are well-formatted"
    ...
    (is (every? #(csa/valid? ::s/ingredient-name %) (map :name (vals bb/base-grains))))))
...
(deftest extract-validity-test
  (testing "Ensure all extract definitions are well-formatted"
    ...
    (is (every? #(csa/valid? ::s/ingredient-name %) (map :name (vals bb/extracts))))))

(deftest hop-validity-test
  (testing "Ensure all hop definitions are well-formatted"
    (is (every? #(csa/valid? ::s/hop-acid %) (map :alpha (vals bb/hops))))
    (is (every? #(csa/valid? ::s/hop-acid %) (map :beta (vals bb/hops))))
    (is (every? #(csa/valid? ::s/ingredient-name %) (map :name (vals bb/hops))))))
...
(deftest yeast-validity-test
  (testing "Ensure all yeast definitions are well-formatted"
    ...
    (is (every? #(csa/valid? ::s/ingredient-name %) (map :name (vals bb/yeasts))))))
